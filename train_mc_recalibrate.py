#
# Train the MC-Dropout U-Net with TRAIN-TIME calibration (Soft-Binned ECE)
#
# Same as train_mc.py (same MC-Dropout net, early stopping, best checkpoint) but
# the loss is  Dice + CE + lambda * SB-ECE  -- a differentiable ECE surrogate that
# makes the network state calibrated confidence natively, instead of relying on a
# post-hoc temperature. (You can still run calibrate_mc.py on top afterwards; the
# two are complementary.)
#
# A custom epoch loop is used because the calibration term needs the target, which
# train_eval.run_epoch's dice+ce signature does not pass. Everything else is reused
# by import (set_seed / pick_device / build_plain_loaders). Default tag is
# 'mcdropout_cal' so test_mc / uncertainty_mc / calibrate_mc run on it via
#   --tag mcdropout_cal
#
# Kaggle: DATA_DIR=/kaggle/input/<prep>/Task04_Hippocampus TASK=Task04_Hippocampus \
#         python train_mc_recalibrate.py --fold 0 --out-dir /kaggle/working/results
#
...
def main():
    ...
    print(f"[{stem}] device={device} dropout_p={args.dropout_p} in_ch={in_channels} "
          f"classes={args.num_classes} loss=dice+ce+{args.cal_weight}*SB-ECE(bins={args.cal_bins})")
