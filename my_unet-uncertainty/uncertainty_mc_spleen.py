#
# Spleen MC-Dropout uncertainty: quantify + visualise
#
# Spleen counterpart of uncertainty_mc.py. Loads a trained checkpoint, keeps
# dropout ACTIVE (enable_dropout), and runs T stochastic forward passes per slice.
# From the T softmax samples it derives:
...
# calibration (foreground ECE + per-class ECE, background excluded -- which is
# critical here, as background is the vast majority of an abdominal CT slice).
#
# Optional recalibration: --temperature T (or the fit saved by
# calibrate_mc_spleen.py, auto-loaded) rescales the softmax as softmax(logits/T).
# With T != 1 the outputs get a '_cal' suffix so raw and recalibrated sit side by
# side.
#
